  ...
  const handleConfirm = async () => {
    try {
      ...
      if (provider) {
        ...
        console.log('Connected Wallet Address:', escrowAddress);
        ...
        if (operation === 'interestRepay') {          
          const requestConfig = {
            ...
            borrower: paymentFrom,
            amount: parseInt(amount, 10),
            operation: ethers.utils.formatBytes32String("Interest")
          };

          txData = await axios.post('http://20.2.203.99:3002/api/repayInterest', requestConfig);
        } else if (operation === 'principalRepay') {
          const requestConfig = {
            contractAddress: contractAddress,
            lender: lenderAddress,
            borrower: paymentFrom,
            amount: parseInt(amount, 10),
            operation: ethers.utils.formatBytes32String("Principal")
          };

          txData = await axios.post('http://20.2.203.99:3002/api/repayPrincipal', requestConfig);
        } else if (operation === 'drawdown') {
          const requestConfig = {
            contractAddress: contractAddress,
          ...
          txData = await axios.post('http://20.2.203.99:3002/api/drawdown', requestConfig);
        }
